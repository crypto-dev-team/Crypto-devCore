...
void PrivacyDialog::on_pushButtonMintzPIV_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("Zerocoin transactions are not supported by Cryptodev."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
...
void PrivacyDialog::on_pushButtonSpendzPIV_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("Zerocoin transactions are not supported by Cryptodev."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        ...
        ui->pushButtonMintzPIV->setToolTip(tr("Zerocoin transactions are not supported by Cryptodev."));

        // Spend zPIV
        ui->pushButtonSpendzPIV->setEnabled(false);
        ui->pushButtonSpendzPIV->setToolTip(tr("Zerocoin transactions are not supported by Cryptodev."));
