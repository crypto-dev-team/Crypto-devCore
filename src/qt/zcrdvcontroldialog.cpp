...
#include "zcrdvcontroldialog.h"
#include "ui_zcrdvcontroldialog.h"
...
std::set<std::string> ZCrdvControlDialog::setSelectedMints;
std::set<CMintMeta> ZCrdvControlDialog::setMints;

ZCrdvControlDialog::ZCrdvControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZCrdvControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZCrdvControlDialog::~ZCrdvControlDialog()
{
    delete ui;
}

void ZCrdvControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZCrdvControlDialog::updateList()
{
...
void ZCrdvControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZCrdvControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZCrdv_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

}

std::vector<CMintMeta> ZCrdvControlDialog::GetSelectedMints()
{
...
void ZCrdvControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
