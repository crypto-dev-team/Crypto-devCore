...
public:
    ...
    bool WriteZCRDVSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZCRDVSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZCRDVSeed_deprecated(uint256& seed);
    bool EraseZCRDVSeed();
    bool EraseZCRDVSeed_deprecated();

    bool WriteZCRDVCount(const uint32_t& nCount);
    bool ReadZCRDVCount(uint32_t& nCount);
