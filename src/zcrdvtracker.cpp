...
CzCRDVTracker::CzCRDVTracker(std::string strWalletFile)
{
...
CzCRDVTracker::~CzCRDVTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzCRDVTracker::Init()
{
...
bool CzCRDVTracker::Archive(CMintMeta& meta)
{
...
bool CzCRDVTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzCRDVTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzCRDVTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzCRDVTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzCRDVTracker::GetSerialHashes()
{
...
CAmount CzCRDVTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzCRDVTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzCRDVTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzCRDVTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzCRDVTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzCRDVTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzCRDVTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzCRDVTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzCRDVTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzCRDVTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzCRDVTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCRDVTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCRDVTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzCRDVTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzCRDVTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzCRDVTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzCRDVTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzcrdv from DB\n", __func__, listDeterministicDB.size());
    }
