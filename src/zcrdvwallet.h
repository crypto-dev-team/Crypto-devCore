...
#ifndef Cryptodev_ZCRDVWALLET_H
#define Cryptodev_ZCRDVWALLET_H
...
public:
    CzCRDVWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZCRDV(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZCRDV(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //Cryptodev_ZCRDVWALLET_H
