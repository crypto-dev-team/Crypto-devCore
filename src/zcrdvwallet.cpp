...
CzCRDVWallet::CzCRDVWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zcrdv seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZCRDVSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZCRDVSeed_deprecated()) {
                    LogPrintf("%s: Updated zCRDV seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zcrdv seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zcrdv wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzCRDVWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZCRDVCount(nCountLastUsed);
    else if (!walletdb.ReadZCRDVCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzCRDVWallet::Lock()
{
    seedMaster = 0;
}

void CzCRDVWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzCRDVWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZCRDV(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzCRDVWallet::LoadMintPoolFromDB()
{
...
void CzCRDVWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzCRDVWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzCRDVWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zcrdvTracker->HasPubcoinHash(pMint.first)) {
...
bool CzCRDVWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZCRDV(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zcrdvTracker which also adds to database
    pwalletMain->zcrdvTracker->Add(dMint, true);
...
void CzCRDVWallet::SeedToZCRDV(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzCRDVWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzCRDVWallet::GenerateDeterministicZCRDV(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzCRDVWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzCRDVWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
